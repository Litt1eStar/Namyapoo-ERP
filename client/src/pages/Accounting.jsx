...
import { Stack, Box, Typography, Button, InputLabel, MenuItem, FormControl, Select } from "@mui/material";
...
const Accounting = () => {
  ...
  const handleFilter = async(e) => {
    ...
    try {
      const res = await fetch(`/api/accounting/filterDate/${month}`)
      ...
      >
        ...
        <Stack width='40%' textAlign='center' mx='auto' mb={5} direction='row'>
          <FormControl fullWidth>
            <InputLabel id="filter-month">เลือกเดือนที่ต้องการ</InputLabel>
            <Select
            ...
            >
              { monthData.map((data) => (
                <MenuItem key={data.value} value={data.value}>{data.display}</MenuItem>
              ))}
            </Select>
          </FormControl>
          <Button sx={{ ml: 5, width: 200}} onClick={handleFilter}>ค้นหา</Button>
        </Stack>
        <Stack>
